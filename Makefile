...
TAXON_GRAPH_URL_PREFIX:=https://zenodo.org/record/1250572/files
...
$(TAXON_CACHE).update $(TAXON_MAP).update:
	...
	zcat $(BUILD_DIR)/names_new.tsv.gz | $(NOMER) append globi-correct | cut -f4,5 | sort | uniq | gzip > $(BUILD_DIR)/names_new_corrected.tsv.gz
	zcat $(BUILD_DIR)/names_new_corrected.tsv.gz | $(NOMER) append globi-enrich | gzip > $(BUILD_DIR)/term_resolved.tsv.gz
	zcat $(BUILD_DIR)/names_new_corrected.tsv.gz | $(NOMER) append globi-globalnames | gzip >> $(BUILD_DIR)/term_resolved.tsv.gz

	zcat $(BUILD_DIR)/term_resolved.tsv.gz | grep -v "NONE" | grep -P "(SAME_AS|SYNONYM_OF)" | cut -f4-12 | gzip > $(BUILD_DIR)/term_match.tsv.gz
	zcat $(BUILD_DIR)/term_resolved.tsv.gz | grep -v "NONE" | grep -P "(SAME_AS|SYNONYM_OF)" | cut -f1,2,4,5 | gzip > $(BUILD_DIR)/term_link_match.tsv.gz
